  ...
  systemd.services.dkm = {
    ...
    serviceConfig = {
      ExecStartPre = lib.optionals hasOpteeOs [
        ''
          ${pkgs.runtimeShell} -c '
            deadline=$((${pkgs.coreutils}/bin/date +%s + ${toString libdogecoinTaWaitTimeoutSeconds}))
            while [ ! -f /lib/optee_armtz/${libdogecoinTaUuid}.ta ]; do
              if [ "$(${pkgs.coreutils}/bin/date +%s)" -ge "$deadline" ]; then
                echo "libdogecoin OP-TEE TA not installed at /lib/optee_armtz/${libdogecoinTaUuid}.ta after ${toString libdogecoinTaWaitTimeoutSeconds}s" >&2
                exit 1
              fi
