...
' /etc/nixos/*.nix
      ...
      if [ ! -f /opt/first-boot-done ]; then
        ...
        ${pkgs.nix}/bin/nix-channel --add https://github.com/edtubbs/nixpkgs/archive/nixos-24.11-optee.tar.gz nixos
        echo "Adding dogebox nix channel..."
        ${pkgs.nix}/bin/nix-channel --add https://github.com/edtubbs/dogebox-nur-packages/archive/${dbxRelease}.tar.gz dogebox
