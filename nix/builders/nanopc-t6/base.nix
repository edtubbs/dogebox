...

{
  ...
  # build-time. iwd is intentionally NOT used — it conflicts with
  # `create_ap` on this rtw88 radio.
  ...
  # `dogebox-ap0-vif.service` below; INTERNET_IFACE is `wlan0` so that once
  # STA associates on the eMMC boot, AP clients get NATed upstream
  # (`SHARE_METHOD` defaults to `nat`). `FREQ_BAND = "2.4"` keeps AP and
  # STA on the same band — rtw88 concurrent AP+STA requires this.
  services.create_ap = {
  ...
  systemd.services."wpa_supplicant-wlan0" = {
    ...
    # their lifecycles. Upstream already sets `Restart = "always"` which is
    # what we want when the GUI rewrites creds — no need to override.
    bindsTo = lib.mkForce [ ];
    wants = [ "sys-subsystem-net-devices-wlan0.device" ];
    after = [
      "dogebox-boot-mode.service"
